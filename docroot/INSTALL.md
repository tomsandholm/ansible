...
- A populated `users.list` with the users who are allowed to register keys
...
sudo cp index.php pubkey-watcher.sh update-users-list.py users.list /var/www/html/
...
## 3. Configure `users.list`

Edit `/var/www/html/users.list` and add every user who should be able to register a key. Each user must already exist in the file before the watcher can update their key.
...
The `pub_key` column can be empty initially; it will be filled in when the user submits a key through the web form.

## 4. Set permissions

The web server needs to write to `pubkey/`. The watcher needs to read drop files and update `users.list`.

### nginx + php-fpm (typical)

```bash
sudo chown -R www-data:www-data /var/www/html/pubkey /var/www/html/users.list
sudo chmod 750 /var/www/html/pubkey
sudo chmod 640 /var/www/html/users.list
```

If the watcher runs as root (default in the provided unit file), root can update `users.list` even when it is owned by `www-data`. Ensure the web server can still write new files into `pubkey/`.
...
In that case, make sure `www-data` can write to `users.list`:

```bash
sudo chown www-data:www-data /var/www/html/users.list
sudo chmod 640 /var/www/html/users.list
```
...
3. Submit a username that exists in `users.list` and a valid SSH public key.
...
# users.list should contain the new key
grep jdoe /var/www/html/users.list
...
Username,Public Key
jdoe,ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAABgQDtest user@host
EOF

sudo python3 /var/www/html/update-users-list.py /tmp/test.csv /var/www/html/users.list jdoe
grep jdoe /var/www/html/users.list
```
...
### Watcher does not update `users.list`

- Confirm the service is running: `systemctl status pubkey-watcher`
- Ensure the username exists in `users.list`
...
- Username not found in `users.list`
